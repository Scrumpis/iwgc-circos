...
[[ ! -f $IWGC_CIRCOS_SIF ]] && { echo "Missing required IWGC Circos container"; exit 1; }
...
if [[ ! -f ${FASTA}.fai ]]; then
  singularity exec ${IWGC_CIRCOS_SIF} samtools faidx ${FASTA}
fi
...
if [[ $INCLUDE_TELOMERE == true ]]; then
  singularity exec ${IWGC_CIRCOS_SIF} quartet.py te -i ${FASTA} -c plant -m 50 -p ${FASTA} --noplot
...
if [[ $INCLUDE_GENE == true || $INCLUDE_REPEAT == true || $INCLUDE_INTACT == true || $INCLUDE_GC == true || $INCLUDE_LTRDATING == true ]]; then
  if [[ "$USE_SLIDING" == true ]]; then
    ...
    singularity exec ${IWGC_CIRCOS_SIF} bedtools makewindows -g ${FASTA}_chrs.fai -w ${WINDOW} -s ${STEP} | \
    awk -v W=${WINDOW} '{ if (($3 - $2) >= (W / 2)) print }' > ${FASTA}_windows.bed
  else
    echo "Generating fixed windows with size $WINDOW"
    singularity exec ${IWGC_CIRCOS_SIF} bedtools makewindows -g ${FASTA}_chrs.fai -w ${WINDOW} > ${FASTA}_windows.bed
  fi
...
if [[ $INCLUDE_GENE == true ]]; then
  ...
  singularity exec ${IWGC_CIRCOS_SIF} bedtools coverage -sorted -a ${FASTA}_windows.bed -b ${GENES}_genes_coords.bed | \
...
if [[ $INCLUDE_REPEAT == true ]]; then
  ...
  singularity exec ${IWGC_CIRCOS_SIF} bedtools coverage -sorted -a ${FASTA}_windows.bed -b ${FASTA}_repeat_coords.bed | \
...
if [[ $INCLUDE_INTACT == true ]]; then
  ...
  for gff in $output_files; do
    ...
    TEMP_FILES+=("$coords")
    singularity exec ${IWGC_CIRCOS_SIF} bedtools coverage -sorted -a ${FASTA}_windows.bed -b $coords | \
...
if [[ $INCLUDE_GC == true ]]; then
  singularity exec ${IWGC_CIRCOS_SIF} bedtools nuc -fi ${FASTA} -bed ${FASTA}_windows.bed > gc_content.bed
...
if [[ $INCLUDE_LTRDATING == true ]]; then
  ...
  singularity exec ${IWGC_CIRCOS_SIF} bedtools map -a ${FASTA}_windows.bed -b ${FASTA}_LTR_insertion.bed -c 4 -o mean > ${FASTA}_LTR_age.bed
